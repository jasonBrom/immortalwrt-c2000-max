...
static u_int32_t app_filter_hook_gateway_handle(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (OAF_CT_TAG(ct) != 0)
	{
		app_id = OAF_CT_TAG(ct) & 0xffff;
		u_int32_t orig_tag = OAF_CT_TAG(ct);
		// 1: drop , 0: accept
		int ct_action = (NF_DROP_BIT == (OAF_CT_TAG(ct) & NF_DROP_BIT)) ? 1 : 0;
		flow.ignore = (NF_IGNORE_BIT == (OAF_CT_TAG(ct) & NF_IGNORE_BIT)) ? 1 : 0;
		...
		if (app_id > 1000 && app_id < 32000)
		{
			...
			if (check_app_action_changed(ct_action, app_id, client)){
				if (ct_action) // drop --> accept
					OAF_CT_TAG(ct) &= ~NF_DROP_BIT;
				else
					OAF_CT_TAG(ct) |= NF_DROP_BIT;
				ct_action = !ct_action;
				AF_LMT_DEBUG("update appid %d action to %s, secmark = %x-->%x\n",
					 app_id, ct_action ? "drop" : "accept", orig_tag, OAF_CT_TAG(ct));
			}
			...
			if (OAF_CT_TAG(ct) & NF_CLIENT_HELLO_BIT) {
	...
	if (total_packets > g_max_dpi_packets) {
		OAF_CT_TAG(ct) = (OAF_CT_TAG(ct) & 0xFFFF0000) | OAF_UNKNOWN_APPID;
		return NF_ACCEPT;
	}

	/* Balanced/precise profiles keep an unknown flow on CPU until DPI decides
	 * ALLOW/BLOCK. Seamless mode deliberately does not set this packet-local
	 * bit: it only inspects packets that naturally traverse the CPU before PPE
	 * or the software flowtable binds the connection. A miss is therefore
	 * recorded as unknown instead of extending the slow path. */
	...
	if (g_oaf_filter_enable && g_disable_quic && af_match_quic(&flow) && match_app_filter_user(client)){
		OAF_CT_TAG(ct) = (OAF_CT_TAG(ct) & 0xFFFF0000) | (APPID_QUIC & 0xFFFF);
		OAF_CT_TAG(ct) |= NF_DROP_BIT;
	...
	if (flow.client_hello) {
		OAF_CT_TAG(ct) |= NF_CLIENT_HELLO_BIT;
	}
	else {
		OAF_CT_TAG(ct) &= ~NF_CLIENT_HELLO_BIT;
	}
	...
	OAF_CT_TAG(ct) = (OAF_CT_TAG(ct) & 0xFFFF0000) | (flow.app_id & 0xFFFF);
	if (flow.feature && flow.feature->ignore){
