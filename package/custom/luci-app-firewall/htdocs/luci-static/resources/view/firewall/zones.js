...
return view.extend({
	...
	load() {
		return Promise.all([
			...
			L.resolveDefault(this.callSystemBoard(), {})
		]);
	},
	...
	renderZones([ctHelpers, fwDefaults, boardInfo]) {
		let m, s, o, out;
		...
		if (L.hasSystemFeature('offloading') && !c2000max) {
		...
		else if (L.hasSystemFeature('offloading') && c2000max) {
			o = s.option(form.DummyValue, '_c2000max_offloading', _('Routing/NAT Offloading'));
			o.rawhtml = true;
			o.cfgvalue = function() {
				return '<a href="%s">%s</a>'.format(
					L.url('admin/network/turboacc'),
					'请在 TurboACC 网络加速页面统一设置，避免软件流量分载与 MediaTek HNAT 同时启用。'
				);
			};
		}
