...
const pkg = {
	...
	// 512 MiB-class routers need an explicit management-plane reserve.  ZRAM
	// increases swap capacity but does not make list expansion/sorting free.
	low_ram_limit: 805306368,
	low_ram_reserve: 100663296,
	low_ram_parallel_cap: 2,
...
function download_lists() {
	...
	let ram_total = get_ram_total();
	let low_ram = ram_total > 0 && ram_total <= pkg.low_ram_limit;
	...
	if (low_ram && task_cap > pkg.low_ram_parallel_cap) {
		task_cap = pkg.low_ram_parallel_cap;
		push(status_data.warnings, {
			code: 'warningParallelDownloadsThrottled',
			info: '' + task_cap
		});
	}
	if (free_mem) {
		// Keep enough headroom for rpcd/uhttpd/dnsmasq.  SwapFree is included
		// in free_mem by the upstream helper, so subtracting the reserve is
		// intentionally conservative during a large first-time list build.
		let budget = free_mem;
		if (low_ram)
			budget = (budget > pkg.low_ram_reserve)
				? budget - pkg.low_ram_reserve : 0;
		if (budget < base + slot_ram) {
			push(status_data.errors, { code: 'errorTooLittleRam', info: '' + free_mem });
			return false;
		}
		let affordable = int((budget - base) / slot_ram);
