...
typedef struct at_port_instance {
    ...
    /*
     * Serialize one complete AT transaction per port.  write_mutex only
     * protects write(2); without this lock a second caller can replace
     * current_response while the first caller is still waiting for its reply.
     */
