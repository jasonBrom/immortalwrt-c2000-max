...
int send_at_command_with_response(at_port_instance_t *port, const char *cmd, int timeout, const char *end_flag, int is_raw, at_response_t *response) {
    ...
    /*
     * Always hand the caller a defined failure object, including when the TTY
     * vanished before it could be opened during a modem USB re-enumeration.
     */
...
int send_at_command_only(at_port_instance_t *port, const char *cmd, int is_raw) {
    ...
    int result = written == (ssize_t)send_len ? 0 : -1;
    pthread_mutex_unlock(&port->transaction_mutex);
    return result;  // Success - command sent
}
