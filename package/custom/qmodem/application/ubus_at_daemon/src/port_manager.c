...
at_port_instance_t *create_port_instance(const char *port_path) {
    ...
    pthread_mutex_init(&port->transaction_mutex, NULL);
...
void destroy_port_instance(at_port_instance_t *port) {
    ...
    /*
     * Unlink first so no new ubus request can discover this instance.  The
     * monitor holds ports_mutex while walking the list, so this also waits for
     * any in-flight monitor pass before the instance can be destroyed.
     */
    pthread_mutex_lock(&g_daemon_ctx.ports_mutex);
    ...
    /*
     * Wait for the complete send/wait transaction, not merely a write, before
     * stopping the reader and closing its descriptor.
     */
    pthread_mutex_lock(&port->transaction_mutex);
    close_at_port(port);
    ...
    pthread_mutex_unlock(&port->transaction_mutex);

    // Destroy mutexes
    pthread_mutex_destroy(&port->transaction_mutex);
...
void *port_monitor_thread_func(void *arg) {
    while (!g_daemon_ctx.monitor_should_stop) {
        ...
        while (current) {
            /*
             * Keep ports_mutex only as lifetime protection.  Never wait here
             * for a long AT transaction (CGACT can take about 35 seconds).
             * This keeps the monitor from extending the global list critical
             * section; shell clients separately serialize synchronous ubus
             * requests before they enter the daemon's single uloop thread.
             * Holding ports_mutex makes the trylock target safe from destroy.
             */
            if (pthread_mutex_trylock(&current->transaction_mutex) == 0) {
                check_and_reconnect_port(current);
                pthread_mutex_unlock(&current->transaction_mutex);
            }
            current = current->next;
        }
