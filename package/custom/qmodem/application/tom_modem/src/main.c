...
static void signal_cleanup(int sig __attribute__((unused)))
{
    clean_up(sig);
    /*
     * Returning from SIGINT/SIGTERM leaves the process running with a closed
     * transport and also lets main() report success.  Do not run atexit
     * cleanup a second time after the signal handler has already released the
     * transport.
     */
    _exit(128 + sig);
}
...
int main(int argc, char *argv[])
{
    ...
    int result = parse_user_input(argc, argv, profile);
    if (result != SUCCESS) {
    ...
    if (result != SUCCESS)
    {
        ...
        /*
         * The caller uses the exit status to distinguish a completed AT
         * transaction from a timeout or transport failure.  The old
         * self-SIGINT path returned here and main() then unconditionally
         * reported SUCCESS, causing QModem to accept failed CGDCONT/CGACT
