...
[ ! -f "${SCRIPT_DIR}/fm350.sh" ] || source "${SCRIPT_DIR}/fm350.sh"
...
get_connect_status()
{
    ...
    if [ "$driver" = "mtk_pcie" ]; then
        ...
        config=$(qmodem_mbim_run "$mbim_port" \
            umbim -d "$mbim_port" config 2>/dev/null) || config=""
        ipv4=$(printf '%s\n' "$config" | grep "ipv4address:" | awk '{print $2}' | cut -d'/' -f1)
        ipv6=$(printf '%s\n' "$config" | grep "ipv6address:" | awk '{print $2}' | cut -d'/' -f1)
        ...
        result=`at  $at_port $at_cmd | grep $expect|tr '\r' '\n'`
	        if { [ "$vendor" = "fibocom" ] || [ "$manufacturer" = "fibocom" ]; } &&
	           [ "$platform" = "mediatek" ]; then
	            local status_cid status_candidates cached_cid configured_cid
	            local status_section
	            local status_context_response status_apn active_candidates
	            configured_cid="$pdp_index"
	            status_section="${modem_config:-$config_section}"
	            cached_cid="$(uci -q get "qmodem.${status_section}.fm350_last_good_cid")"
	            status_context_response="$(at_timeout "$at_port" "AT+CGDCONT?" 8 2>&1)"
	            fm350_contexts_valid=0
	            fm350_contexts=""
	            if ! fm350_response_has_error "$status_context_response" &&
	               printf '%s\n' "$status_context_response" | grep -q '+CGDCONT:'; then
	                fm350_contexts="$(fm350_context_records "$status_context_response")"
	                [ -n "$fm350_contexts" ] && fm350_contexts_valid=1
	            fi
	            active_candidates="$(echo "$result" | tr -d "\r" |
	                awk -F'[,:]' '$3 == 1 {gsub(/[[:space:]]/, "", $2); print $2}')"
	            status_candidates="$configured_cid $cached_cid 0 3"
	            [ "$fm350_contexts_valid" != 1 ] ||
	                status_candidates="$status_candidates $active_candidates"
	            fm350_status_seen=""
	            for status_cid in $status_candidates; do
	                fm350_is_uint "$status_cid" || continue
                case " $fm350_status_seen " in
                    *" $status_cid "*) continue ;;
	                esac
	                fm350_status_seen="$fm350_status_seen $status_cid"
	                if [ "$fm350_contexts_valid" = 1 ]; then
	                    status_apn="$(fm350_context_apn "$status_cid" 2>/dev/null)"
	                    fm350_candidate_apn_allowed "$status_apn" || continue
	                fi
	                if fm350_query_addresses "$status_cid" &&
	                   fm350_address_matches_pdp_type; then
	                    connect_status="Yes"
                    break
                fi
            done
            pdp_index="$configured_cid"
