...
get_sms(){
    ...
    if [ ! -f $cache_file ] || [ $(($current_time - $file_time)) -gt $cache_timeout ]; then
        touch $cache_file
        qmodem_tom_modem "$at_port" -o r > "$cache_file"
...
case $method in
    ...
    "delete_sms")
        ...
        [ -n "$sms_at_port" ] && at_port=$sms_at_port
        for i in $index; do
            qmodem_tom_modem "$at_port" -o d -i "$i"
            ret=$?
    ...
    "send_raw_pdu")
        cmd=$3
        [ -n "$sms_at_port" ] && at_port=$sms_at_port
        res=$(qmodem_tom_modem "$at_port" -o s -p "$cmd")
        ret=$?
        json_select result
        if [ "$ret" == 0 ]; then
            json_add_string status "1"
    ...
    "send_sms")
        ...
        [ -n "$sms_at_port" ] && at_port=$sms_at_port
        qmodem_sms_tool_q "$at_port" send "$phone_number" "$message_content" > /dev/null
        ret=$?
        json_select result
        if [ "$ret" == 0 ]; then
            json_add_string status "1"
