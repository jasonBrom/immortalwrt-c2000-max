...
base_info()
{
    ...
    # Revision（固件版本）. QModem-custom uses Quectel's dedicated QGMR
    # query; retain ATI as a compatibility fallback for older firmware.
    at_command="AT+QGMR"
    revision=$(at $at_port $at_command | sed -n '2p' | sed 's/\r//g')
    if [ -z "$revision" ] || [ "$revision" = "ERROR" ]; then
        at_command="ATI"
