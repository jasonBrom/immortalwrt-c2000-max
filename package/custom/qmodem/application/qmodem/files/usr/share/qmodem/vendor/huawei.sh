...
# Extra MT5700 fields used by the Kunpeng APP cellular-detail page.  The
# normal qmodem info response is cached for 10 seconds; keep these slower,
# mostly-static AT queries in a separate 60-second cache so APP polling does
# not turn them into a high-frequency serial workload.  All at() calls still
# use QModem's shared per-port transaction lock.
_c2000max_huawei_detail_reset()
{
    c2000max_detail_band=""
    c2000max_detail_dl_fcn=""
    c2000max_detail_ul_fcn=""
    c2000max_detail_dlbw=""
    c2000max_detail_ulbw=""
    c2000max_detail_band1=""
    c2000max_detail_dl_fcn1=""
    c2000max_detail_ul_fcn1=""
    c2000max_detail_dlbw1=""
    c2000max_detail_ulbw1=""
    c2000max_detail_cqi=""
    c2000max_detail_ambr_dl=""
    c2000max_detail_ambr_ul=""
}

_c2000max_huawei_detail_load()
{
    local target_rat="$1"
    local cache_file="/tmp/cache_c2000max_huawei_detail_${config_section}"
    local cache_time="" cache_rat="" now age
    local hfreq_line="" parsed="" result="" cid=""
    local temp_file="${cache_file}.$$"

    _c2000max_huawei_detail_reset
    now="$(date +%s)"
    if [ -r "$cache_file" ]; then
        IFS='|' read -r cache_time cache_rat \
            c2000max_detail_band c2000max_detail_dl_fcn \
            c2000max_detail_ul_fcn c2000max_detail_dlbw \
            c2000max_detail_ulbw c2000max_detail_band1 \
            c2000max_detail_dl_fcn1 c2000max_detail_ul_fcn1 \
            c2000max_detail_dlbw1 c2000max_detail_ulbw1 \
            c2000max_detail_cqi c2000max_detail_ambr_dl \
            c2000max_detail_ambr_ul < "$cache_file" || true
        case "$cache_time" in
            ''|*[!0-9]*) cache_time=0 ;;
        esac
        age=$((now - cache_time))
        if [ "$cache_rat" = "$target_rat" ] &&
           [ "$age" -ge 0 ] && [ "$age" -le 60 ]; then
            return 0
        fi
    fi

    _c2000max_huawei_detail_reset
    hfreq_line="$(at "$at_port" "AT^HFREQINFO?" 2>/dev/null |
        awk -F',' -v wanted="$target_rat" '
            {
                rat=$2
                gsub(/[^0-9]/, "", rat)
                if (rat == wanted) { print; exit }
            }')"
    if [ -n "$hfreq_line" ]; then
        parsed="$(printf '%s\n' "$hfreq_line" | awk -F',' '
            ...
            }
            {
                printf "%s %s %s %s %s %s %s %s %s %s\n", \
                    clean($3), clean($4), clean($7), clean($6), \
                    clean($9), clean($10), clean($11), clean($14), \
                    clean($13), clean($16)
            }')"
        set -- $parsed
        c2000max_detail_band="${1:-}"
        c2000max_detail_dl_fcn="${2:-}"
        c2000max_detail_ul_fcn="${3:-}"
        c2000max_detail_dlbw="${4:-}"
        c2000max_detail_ulbw="${5:-}"
        c2000max_detail_band1="${6:-}"
        c2000max_detail_dl_fcn1="${7:-}"
        c2000max_detail_ul_fcn1="${8:-}"
        c2000max_detail_dlbw1="${9:-}"
        shift 9 2>/dev/null || true
        c2000max_detail_ulbw1="${1:-}"
        [ "$c2000max_detail_band" = "-" ] && c2000max_detail_band=""
        [ "$c2000max_detail_dl_fcn" = "-" ] && c2000max_detail_dl_fcn=""
        [ "$c2000max_detail_ul_fcn" = "-" ] && c2000max_detail_ul_fcn=""
        [ "$c2000max_detail_dlbw" = "-" ] && c2000max_detail_dlbw=""
        [ "$c2000max_detail_ulbw" = "-" ] && c2000max_detail_ulbw=""
        [ "$c2000max_detail_band1" = "-" ] && c2000max_detail_band1=""
        [ "$c2000max_detail_dl_fcn1" = "-" ] && c2000max_detail_dl_fcn1=""
        [ "$c2000max_detail_ul_fcn1" = "-" ] && c2000max_detail_ul_fcn1=""
        [ "$c2000max_detail_dlbw1" = "-" ] && c2000max_detail_dlbw1=""
        [ "$c2000max_detail_ulbw1" = "-" ] && c2000max_detail_ulbw1=""
    fi

    if [ "$target_rat" = 7 ]; then
        for cid in 8 1; do
            result="$(at "$at_port" "AT^DSAMBR=$cid" 2>/dev/null |
                awk -F',' '/DSAMBR:/ {
                    gsub(/[^0-9]/, "", $2)
                    gsub(/[^0-9]/, "", $3)
                    if ($2 != "" || $3 != "") print $2 " " $3
            ...
            set -- $result
            c2000max_detail_ambr_dl="${1:-}"
            c2000max_detail_ambr_ul="${2:-}"
            [ -n "$c2000max_detail_ambr_dl$c2000max_detail_ambr_ul" ] && break
        done

        for cid in 8 1; do
            c2000max_detail_cqi="$(at "$at_port" \
                "AT+CGEQOSRDP=$cid" 2>/dev/null |
                awk -F',' '/CGEQOSRDP:/ {
                    gsub(/[^0-9]/, "", $2); print $2; exit
                }')"
            [ -n "$c2000max_detail_cqi" ] ||
                c2000max_detail_cqi="$(at "$at_port" \
                    "AT+C5GQOSRDP=$cid" 2>/dev/null |
                    awk -F',' '/C5GQOSRDP:/ {
                        gsub(/[^0-9]/, "", $2); print $2; exit
                    }')"
            [ -n "$c2000max_detail_cqi" ] && break
        done
    fi

    ( umask 077
      printf '%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s\n' \
        "$now" "$target_rat" "$c2000max_detail_band" \
        "$c2000max_detail_dl_fcn" "$c2000max_detail_ul_fcn" \
        "$c2000max_detail_dlbw" "$c2000max_detail_ulbw" \
        "$c2000max_detail_band1" "$c2000max_detail_dl_fcn1" \
        "$c2000max_detail_ul_fcn1" "$c2000max_detail_dlbw1" \
        "$c2000max_detail_ulbw1" "$c2000max_detail_cqi" \
        "$c2000max_detail_ambr_dl" "$c2000max_detail_ambr_ul" \
        > "$temp_file"
    ) && mv -f "$temp_file" "$cache_file"
}

_c2000max_huawei_detail_add()
{
    _c2000max_huawei_detail_load "$1"
    [ -n "$c2000max_detail_band" ] &&
        add_plain_info_entry "Band" "$c2000max_detail_band" "Band"
    [ -n "$c2000max_detail_dl_fcn" ] &&
        add_plain_info_entry "DL FCN" "$c2000max_detail_dl_fcn" "DL FCN"
    [ -n "$c2000max_detail_ul_fcn" ] &&
        add_plain_info_entry "UL FCN" "$c2000max_detail_ul_fcn" "UL FCN"
    [ -n "$c2000max_detail_dlbw" ] &&
        add_plain_info_entry "DL Bandwidth" "$c2000max_detail_dlbw" "DL Bandwidth"
    [ -n "$c2000max_detail_ulbw" ] &&
        add_plain_info_entry "UL Bandwidth" "$c2000max_detail_ulbw" "UL Bandwidth"
    [ -n "$c2000max_detail_band1" ] &&
        add_plain_info_entry "Band 1" "$c2000max_detail_band1" "Band 1"
    [ -n "$c2000max_detail_dl_fcn1" ] &&
        add_plain_info_entry "DL FCN 1" "$c2000max_detail_dl_fcn1" "DL FCN 1"
    [ -n "$c2000max_detail_ul_fcn1" ] &&
        add_plain_info_entry "UL FCN 1" "$c2000max_detail_ul_fcn1" "UL FCN 1"
    [ -n "$c2000max_detail_dlbw1" ] &&
        add_plain_info_entry "DL Bandwidth 1" "$c2000max_detail_dlbw1" "DL Bandwidth 1"
    [ -n "$c2000max_detail_ulbw1" ] &&
        add_plain_info_entry "UL Bandwidth 1" "$c2000max_detail_ulbw1" "UL Bandwidth 1"
    [ -n "$c2000max_detail_cqi" ] &&
        add_plain_info_entry "CQI" "$c2000max_detail_cqi" "Channel Quality Indicator"
    [ -n "$c2000max_detail_ambr_dl" ] &&
        add_plain_info_entry "NR5G AMBR DL" "$c2000max_detail_ambr_dl" "NR5G AMBR DL"
    [ -n "$c2000max_detail_ambr_ul" ] &&
        add_plain_info_entry "NR5G AMBR UL" "$c2000max_detail_ambr_ul" "NR5G AMBR UL"
}
    ...
    case "$platform" in
        "unisoc")
            ...
            case "$network_mode" in
                "NR5G-SA Mode"|"EN-DC Mode") _c2000max_huawei_detail_add 7 ;;
                "LTE Mode") _c2000max_huawei_detail_add 6 ;;
            esac
            ;;
        *)
            at_command="AT^MONSC"
