...
LED_STATE_FILE="$(mktemp)"
trap 'rm -f "$LED_STATE_FILE"' EXIT
printf '%s\n' 'config|qmodem_led.main.poll_interval=5' > "$LED_STATE_FILE"
if led_state_needs_watcher; then
	echo 'empty LED configuration incorrectly needs a watcher' >&2
	exit 1
fi
printf '%s\n' 'modem_status|cellular|1|misectel_modem_status|any||1|0||' > "$LED_STATE_FILE"
led_state_needs_watcher || {
	echo 'configured modem LED does not start a watcher' >&2
	exit 1
}
