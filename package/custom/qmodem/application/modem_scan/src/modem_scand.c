...
static int word_list_has(const char *list, const char *word)
{
	const char *p = list;
	...
	while (p && *p) {
		...
		while (*p && !isspace((unsigned char)*p))
			p++;
		len = (size_t)(p - start);
		if (len == word_len && !strncmp(start, word, len))
			return 1;
	}
	return 0;
}

static int role_lock_acquire(void)
{
	int lock_fd;

	lock_fd = open(C2000_ROLE_LOCK, O_RDWR | O_CREAT, 0700);
	if (lock_fd < 0)
		return -1;
...
static void role_lock_release(int lock_fd)
{
	if (lock_fd < 0)
		return;
...
static int at_command(const char *port, const char *cmd, int fast, int timeout, char *out, size_t out_len)
{
	...
	const char *script =
		"QMODEM_AT_BACKEND=ubus; QMODEM_AT_READY_FD=3; "
		"export QMODEM_AT_BACKEND QMODEM_AT_READY_FD; "
	...
	close(readyfd[1]);
	fcntl(pipefd[0], F_SETFL, fcntl(pipefd[0], F_GETFL, 0) | O_NONBLOCK);
	fcntl(readyfd[0], F_SETFL, fcntl(readyfd[0], F_GETFL, 0) | O_NONBLOCK);
	queue_start = time(NULL);
	for (;;) {
		fd_set rfds;
		struct timeval tv = { .tv_sec = 0, .tv_usec = 100000 };
		char tmp[512];
		...
		FD_SET(pipefd[0], &rfds);
		if (!command_ready) {
			FD_SET(readyfd[0], &rfds);
			if (readyfd[0] > maxfd)
				maxfd = readyfd[0];
		}
		if (select(maxfd + 1, &rfds, NULL, NULL, &tv) > 0) {
			if (!command_ready && FD_ISSET(readyfd[0], &rfds)) {
				char marker;
				if (read(readyfd[0], &marker, 1) == 1) {
					command_ready = 1;
		...
		if (r == pid)
			break;
		now = time(NULL);
		if ((!command_ready && now - queue_start >= QMODEM_AT_QUEUE_TIMEOUT) ||
		    (command_ready && timeout > 0 && now - command_start >= timeout)) {
			kill(-pid, SIGTERM);
			usleep(200000);
			if (waitpid(pid, &status, WNOHANG) == 0)
				kill(-pid, SIGKILL);
			...
			close(pipefd[0]);
			close(readyfd[0]);
			log_msg(LOG_L_WARN, "%s timeout port=%s cmd=%s",
				command_ready ? "AT command" : "AT queue", port, cmd);
			return -2;
		}
	}
	close(pipefd[0]);
...
static void normalize_model_name(char *name, size_t len)
{
	...
	else if (strstr(lower, "rw350r")) snprintf(name, len, "rw350r-gl");
	else if (strstr(lower, "fm350r")) snprintf(name, len, "fm350r-gl");
	else if (strstr(lower, "fm350-gl-00")) snprintf(name, len, "fm350-gl-00");
	else if (strstr(lower, "fm350")) snprintf(name, len, "fm350-gl");
...
static int extract_candidate_lines(const char *res, struct str_list *names)
{
	...
	for (line = strtok_r(copy, "\n", &save); line; line = strtok_r(NULL, "\n", &save)) {
		...
		} else if (!strncasecmp(line, "Model:", sizeof("Model:") - 1)) {
			/* Some Quectel firmware exposes the usable model only through ATI. */
			char *p = strchr(line, ':');
			if (p) {
				p++;
				trim(p);
...
{
	/* ATI is intentionally last: keep the standard model commands fast, but
	 * retain QModem-custom's fallback for firmware returning Model: only. */
	const char *cmds[] = { "AT+CGMM", "AT+CGMM?", "AT+GMM", "ATI" };
...
static void reload_network_serialized(void)
{
	char role[16] = "", mode[32] = "";
	int role_lock_fd;

	role_lock_fd = role_lock_acquire();
	if (role_lock_fd < 0) {
		log_msg(LOG_L_ERR,
			"failed to acquire C2000 port-role lock for QModem reload");
		return;
	}
	uci_get("c2000max.ethernet.role", role, sizeof(role));
	uci_get("c2000max.ethernet.wan_mode", mode, sizeof(mode));
	if (access(C2000_SWITCHING, F_OK) == 0 ||
	    (!strcmp(role, "wan") && !strcmp(mode, "ethernet_only"))) {
		log_msg(LOG_L_INFO,
			"skip QModem reload while C2000 uplink policy keeps cellular quiesced");
	} else {
		/*
		 * Keep ROLE through the complete service reload.  A port transaction
		 * must not pass its qmodem_stop barrier while this scanner can still
		 * restart the service from a just-discovered modem.
		 */
		reload_network();
	}
	role_lock_release(role_lock_fd);
}
...
static int add_modem(const char *slot, const char *slot_type)
{
	...
	int existed, c2000max_changed = 0, role_lock_fd = -1;
	...
	role_lock_fd = role_lock_acquire();
	if (role_lock_fd < 0) {
		log_msg(LOG_L_ERR, "failed to acquire C2000 port-role lock");
		goto out_fail;
	}
	...
	if (existed) {
		...
		char board_role[16] = "", board_mode[32] = "";
		char board_override[8] = "", selected[128] = "";
		char overridden[2048] = "", saved[64] = "";
		int board_balanced;
		int modem_count = 0;
		...
		uci_get("c2000max.ethernet.role", board_role, sizeof(board_role));
		uci_get("c2000max.ethernet.wan_mode", board_mode, sizeof(board_mode));
		uci_get("c2000max.ethernet.qmodem_override_active",
			board_override, sizeof(board_override));
		uci_get("c2000max.ethernet.resolved_cellular_modem",
			selected, sizeof(selected));
		board_balanced = !strcmp(board_role, "wan") &&
			!strcmp(board_mode, "ethernet_5g_balance") &&
			!strcmp(board_override, "1");
		snprintf(key, sizeof(key), "qmodem.%s.enable_dial", section);
		uci_set(key, board_balanced && strcmp(section, selected) ? "0" : "1");
		if (board_balanced) {
			snprintf(key, sizeof(key),
				"c2000max.ethernet.qmodem_saved_%s", section);
			if (uci_get(key, saved, sizeof(saved)) || !saved[0])
				/* Outside the board override a newly scanned modem is
				 * created with enable_dial=1, so preserve that baseline. */
				uci_set(key, "1");
			uci_get("c2000max.ethernet.qmodem_overridden_devices",
				overridden, sizeof(overridden));
			if (!word_list_has(overridden, section))
				uci_add_list(
					"c2000max.ethernet.qmodem_overridden_devices",
					section);
			c2000max_changed = 1;
		}
			snprintf(key, sizeof(key), "qmodem.%s.soft_reboot", section); uci_set(key, "1");
			/* AT ports are request/response streams.  New modems use the shared
			 * ubus-at-daemon reader by default so UI, SMS, LED and dial helpers
...
static void load_config_defaults(void)
{
	...
	/*
	 * All scanner AT calls use the synchronous ubus backend and its global
	 * pre-submit lock.  Parallel probe children start their outer deadline
	 * before acquiring that lock, so later ttyUSB ports could be killed while
	 * merely waiting and FM350's common ttyUSB3 control port would disappear
	 * from valid_at_ports.  Keep scanner work serial until the daemon exposes
	 * genuinely deferred/asynchronous ubus requests.
	 */
	g.scan_workers = 1;
	g.at_probe_workers = 1;
