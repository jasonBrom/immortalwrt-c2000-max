...
return view.extend({
	...
	o = s.option(form.ListValue, 'at_backend', _('AT Backend'));
	o.description = _('Auto uses the serialized ubus AT queue for all modems. Select a direct backend only for troubleshooting.');
	o.value('auto', _('Auto'));
	o.value('ubus', _('Ubus AT queue'));
	o.value('sms_tool_q', 'sms_tool_q');
	o.value('tom_modem', 'tom_modem');
	o.default = 'auto';
	o.rmempty = false;
	o.modalonly = true;
	...
	o = s.option(form.Flag, 'fm350_pdp_auto', _('FM350 Automatic PDP Context'));
	o.description = _('Try the configured CID first, then discover a safe working context without overwriting IMS or emergency contexts.');
	o.default = '1';
	o.rmempty = false;
	o.modalonly = true;

	o = s.option(form.Flag, 'fm350_legacy_route_guess', _('FM350 Legacy Route Guess'));
	o.description = _('Only for old firmware where both CGCONTRDP and DHCP are incomplete. Guesses a /24 mask and .1 gateway; keep disabled for normal use.');
	o.default = '0';
	o.rmempty = false;
	o.modalonly = true;
	o.depends('fm350_pdp_auto', '1');
