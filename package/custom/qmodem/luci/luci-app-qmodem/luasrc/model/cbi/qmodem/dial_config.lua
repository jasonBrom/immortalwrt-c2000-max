...
at_backend = s:taboption("advanced", ListValue, "at_backend", translate("AT Backend"))
at_backend.description = translate("Auto uses the serialized ubus AT queue for all modems. Select a direct backend only for troubleshooting.")
at_backend.default = "auto"
at_backend.rmempty = true
at_backend:value("auto", translate("Auto"))
at_backend:value("ubus", translate("Ubus AT queue"))
at_backend:value("sms_tool_q", "sms_tool_q")
at_backend:value("tom_modem", "tom_modem")
...
fm350_pdp_auto = s:taboption("advanced", Flag, "fm350_pdp_auto", translate("FM350 Automatic PDP Context"))
fm350_pdp_auto.description = translate("Try the configured CID first, then discover a safe working context without overwriting IMS or emergency contexts.")
fm350_pdp_auto.default = "1"
fm350_pdp_auto.rmempty = false

fm350_legacy_route_guess = s:taboption("advanced", Flag, "fm350_legacy_route_guess", translate("FM350 Legacy Route Guess"))
fm350_legacy_route_guess.description = translate("Only for old firmware where both CGCONTRDP and DHCP are incomplete. Guesses a /24 mask and .1 gateway; keep disabled for normal use.")
fm350_legacy_route_guess.default = "0"
fm350_legacy_route_guess.rmempty = false
fm350_legacy_route_guess:depends("fm350_pdp_auto", "1")
