	...
	renderStatus: function(status) {
		...
		const rows = [
			...
			[ '5G 上网接口', wanMode === 'ethernet_5g_balance' ?
				...
				'未启用' ],
			[ '网络 / WAN 配置 / 服务启动策略', '%s / %s / %s'.format(
				networkConverged ? '已收敛' : '未收敛',
				uplinkConsistent ? '已收敛' : '未收敛',
				servicesConsistent ? '已应用' : '待应用') ],
	...
	setControls: function(status) {
		...
		if (lan)
			lan.disabled = !consistent || switching ||
				(status.role === 'lan' && networkConverged);
		if (lan)
	...
	render: function(status) {
		...
		const root = E('div', {}, [
			...
			E('div', { 'class': 'cbi-map-descr' },
				'切换对象是 C2000-MAX 唯一的 2.5G 物理网口 eth1。WAN 可选择仅使用网口宽带，或让网口与 5G 分流。LAN/WAN 都使用各自的 MediaTek HNAT 硬件端点；QModem 拨号慢或 mwan3 尚未启动不会再阻止网络生效。'),
			...
			E('div', { 'class': 'cbi-section' }, [
				...
				E('div', { 'class': 'cbi-section-descr' },
					'“仅网口宽带”会停止并禁用 QModem 蜂窝拨号；“网口宽带 + 5G 叠加”会自动配置 mwan3，仅按 IPv4 新连接权重近似分流。IPv6 不参与 V24 权重分流，单个 TCP 连接也不会带宽相加。'),
