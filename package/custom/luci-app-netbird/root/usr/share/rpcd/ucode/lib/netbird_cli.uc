...
//   - 2s timeout（热路径防卡死）：所有 popen/system 调用以 timeout 2s 前缀包装。
...
// _with_timeout(cmd) → 拼接 `timeout 2s <cmd>` 或 `<cmd>`（无 timeout 时降级）
...
//   - cli_error     → 退出码 124（timeout 语义）；message = "timeout after 2s"
...
function fetch_status_json(bin_path) {
    ...
    if (r.exit_code == 124)
        return { ok: false, code: 'cli_error', message: 'timeout after 2s' };
    if (r.exit_code != 0) {
...
// ============================================================================
// 2s timeout；找不到对应包返空串。不抛异常。命令不接受用户输入，无需 shell_quote。
